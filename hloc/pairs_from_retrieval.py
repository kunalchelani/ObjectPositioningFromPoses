...
from .utils.parsers import parse_image_lists, parse_image_names_list
from .utils.read_write_model import read_images_binary
from .utils.io import list_h5_names


def parse_names(prefix, names, names_all, with_intrinsics = False):
    if prefix is not None:
        if not isinstance(prefix, str):
            prefix = tuple(prefix)
        names = [n for n in names_all if n.startswith(prefix)]
    elif names is not None and isinstance(names, (str, Path)):
        names = parse_image_names_list(names, with_intrinsics)
...
def main(descriptors, output, num_matched,
         ...
         db_prefix=None, db_list=None, db_model=None, db_descriptors=None, with_intrinsics = False):
    ...
    if db_model:
        ...
        db_names = parse_names(db_prefix, db_list, db_names_h5, with_intrinsics)
    if len(db_names) == 0:
        raise ValueError('Could not find any database image.')
    query_names = parse_names(query_prefix, query_list, query_names_h5, with_intrinsics)
