...
def pose_from_cluster(qname, qinfo, db_ids, db_images, points3D,
                      feature_file, match_file, thresh, use_loftr = False):
    #print(list(feature_file[qname].keys()))
    ...
    for i, db_id in enumerate(db_ids):
        ...
        if not use_loftr:
            ...
            if len(matches) < 1:
                logging.debug(f'No matches found {db_name}.')
                continue
            
            valid = np.where(points3D_ids[matches[:,1]] != -1)[0]
            valid_matches = matches[valid, :]
            num_matches += len(valid)
            #print("Valid matches {}".format(valid_matches.shape))
            
            for j in range(len(valid_matches)):
                
                id_3D = points3D_ids[valid_matches[j,1]]
                kp_idx_to_3D_to_db[valid_matches[j,0]][id_3D].append(i)
                if id_3D not in kp_idx_to_3D[valid_matches[j,0]]:
                    kp_idx_to_3D[valid_matches[j,0]].append(id_3D)
    ...
    #print("using {} 2D-3D point correspondences to get a pose.".format(len(mkpq)))
    ...
    ret = pycolmap.absolute_pose_estimation(mkpq, mp3d, cfg, max_error_px = thresh)
    ret['cfg'] = cfg
...
def main(reference_sfm, queries, retrieval, features, matches, results,
         ransac_thresh=15, covisibility_clustering=False,
         prepend_camera_name=False, return_top_db_failure = False, use_loftr = False):
    ...
    for qname, qinfo in tqdm(queries):
        ...
        if covisibility_clustering:
            ...
            for i, cluster_ids in enumerate(clusters):
                ret, mkpq, mp3d, mp3d_ids, num_matches, map_ = (
                        pose_from_cluster(
                            ...
                            feature_file, match_file, thresh=ransac_thresh, use_loftr=use_loftr))
                #if ret['success']:
                    #print("Cluster Images")
                    #print([db_images[db_id].name for db_id in cluster_ids])
                    #print("Num_inliers : {}".format(ret['num_inliers']))

                if ret['success'] and ret['num_inliers'] > best_inliers:
                    best_cluster = i
                    best_inliers = ret['num_inliers']
                    #print("actually found a pose")
            ...
            if ret['success']:
                #print("actually found a pose")
                poses[qname] = (ret['qvec'], ret['tvec'])
                num_inliers[qname] = ret['num_inliers']
            else:
                closest = db_images[db_ids[0]]
                num_inliers[qname] = 0
                if return_top_db_failure:
